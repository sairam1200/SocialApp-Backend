...
import { GetPreferenceQuery } from "./get-preference.handler";
...
export class GetPreferenceController {
  constructor(
  ...
  public async Get(): Promise<UserPreferenceModel> {
    const result = await this.queryBus.execute(new GetPreferenceQuery());
    return result;
  }
}
