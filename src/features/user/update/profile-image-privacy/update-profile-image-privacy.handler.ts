...
const updateProfileImagePrivacyValidations = Joi.object({
  privacy: Joi.string().valid(...Object.values(ProfileImagePrivacy)).required(),
});
...
export class UpdateProfileImagePrivacyCommandHandler implements ICommandHandler<UpdateProfileImagePrivacyCommand> {
  ...
  public async execute(command: UpdateProfileImagePrivacyCommand): Promise<void> {
    const { model } = command;
    ...
    await this.userRepository.updateUserBiometricPrivacyAsync(user.id, model.privacy);
  }
}
