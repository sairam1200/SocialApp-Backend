...
export class UpdateEmailCommandHandler implements ICommandHandler<UpdateEmailCommand, void> {
  constructor(
  ...
  async execute(command: UpdateEmailCommand): Promise<void> {
    ...
    if (user.lastEmailModifiedAt) {
      ...
      const timeSinceLastChange = Date.now() - new Date(user.lastEmailModifiedAt).getTime();
    ...
    const isEmailInuse = await this.userRepository.isEmailInuseAsync(command.email);
    if (isEmailInuse) {
