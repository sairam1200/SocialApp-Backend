...
export class SendVerificationEmailCommandHandler implements ICommandHandler<SendVerificationEmailCommand> {
  ...
  public async execute(command: SendVerificationEmailCommand): Promise<void> {
    ...
    if (user.emailConfirmed) {
      logger.info(`Verification email requested for already verified user ${user.id}`);
      return;
    }
  ...
  private async sendVerificationEmail(user: any, model: SendVerificationEmailRequestModel): Promise<void> {
    try {
      ...
      await this.emailService.sendTemplatedAsync({
        ...
        context: {
          ...
          securityWarning: "If you didn't create an account with Gaddr, please ignore this email.",
          year: new Date().getFullYear(),
        },
      });

      logger.info(`Verification email sent to user ${user.id} from IP: ${model.ipAddress}, Location: ${location}, Device: ${deviceInfo}`);
    } catch (error) {
      logger.error(`Failed to send verification email for user ${user.id}: ${error instanceof Error ? error.message : JSON.stringify(error)}`, { error });
      throw error;
    }
  }
