...
import { IUserRepository, IUserFollowRepository, ILinkedAccountRepository } from "../../../domain/repositories";
...
export class DiscoverCreatorsQueryHandler implements ICommandHandler<DiscoverCreatorsQuery, { profiles: PublicProfileModel[]; page: number; limit: number; totalResults: number; hasNextPage: boolean }> {
  constructor(
  ...
  async execute(query: DiscoverCreatorsQuery): Promise<{ profiles: PublicProfileModel[]; page: number; limit: number; totalResults: number; hasNextPage: boolean }> {
    await validateDiscoverCreatorsQuery.validateAsync(query);

    const [users, total] = await this.userRepository.getDiscoverCreatorsAsync(query.page, query.limit);
    ...
    const profiles = await Promise.all(
      users.map(async (user: User) => {
        ...
        const followersCount = await this.userFollowRepository.countFollowersAsync(user.id);
        const followingCount = await this.userFollowRepository.countFollowingAsync(user.id);

        let profileImageUrl: string | null = null;
        if (user.biometrics) {
          profileImageUrl = user.biometrics.profileImageUrl || user.biometrics.defaultProfileImageUrl || null;
        }
