...
export class Verify2FACommandHandler implements ICommandHandler<Verify2FACommand, any> {
  ...
  public async execute(command: Verify2FACommand): Promise<any> {
    ...
    this.checkForUnrecognizedDeviceOrIp(model.deviceId, model.userAgent, model.ipAddress);
  ...
  private checkForUnrecognizedDeviceOrIp(deviceId: string, userAgent: string, ipAddress: string) {
    let hasChanged = deviceId !== HttpContext.user['device-id'];
    if (hasChanged) {
      throw new ApplicationException("Login attempt detected from an unrecognized device.");
    }
    ...
    if (hasChanged) {
      const isNewRegion = ipUtil.hasIpChanged(ipAddress, HttpContext.user['ip-address']);
      if (isNewRegion) {
