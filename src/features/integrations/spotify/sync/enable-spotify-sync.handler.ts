  ...
  implements ICommandHandler<EnableSpotifySyncCommand>
{
  ...
  public async execute(
    command: EnableSpotifySyncCommand,
    ...
    logger.info(
      `[SpotifySync] Sync enabled for user ${userId}. Triggering initial import.`,
    );
    ...
    if (userLogin) {
      ...
      logger.warn(
        `[SpotifySync] UserLogin not found for user ${userId}. Cannot trigger import after sync enable.`,
      );
    }
