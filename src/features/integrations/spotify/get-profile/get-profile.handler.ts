...
export class SpotifyProfileQueryHandler implements ICommandHandler<SpotifyProfileQuery> {
  ...
  public async execute(query: SpotifyProfileQuery): Promise<SpotifyProfileModel> {
    ...
    const account = model.userId ? await this.linkedAccountRepository.getByPlatformAndUserIdAsync(PLATFORM, model.userId)
      : model.userName ? await this.linkedAccountRepository.getByPlatformAndUserNameAsync(PLATFORM, model.userName)
        : await this.linkedAccountRepository.getByPlatformAndExternalIdAsync(PLATFORM, model.spotifyId);

    if (!account) {
      throw new NotFoundException("No matching Spotify profile was found based on the provided information.");
    }
