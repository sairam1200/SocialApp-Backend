...
export class PinterestProfileController {
  ...
  public async Me(@Res() res: Response): Promise<Response | void> {
    ...
    const result = await this.commandBus.execute(
      new PinterestProfileQuery({ model: { userId } }),
    );
  ...
  public async Profile(
    ...
    const params = [userId, userName, pinterestId].filter(
      (param) => param !== undefined && param !== null,
    );
    if (params.length > 1) {
      throw new BadRequestException(
        'Only one of the following query parameters should be provided: userId, userName, or pinterestId.',
      );
    }

    const result = await this.commandBus.execute(
      new PinterestProfileQuery({ model: { userId, userName, pinterestId } }),
    );
