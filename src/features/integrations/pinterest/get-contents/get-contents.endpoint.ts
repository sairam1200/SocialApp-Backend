...
import { PinterestContentsQuery } from "./get-contents.handler";
...
export class PinterestContentsController {
  ...
  @ApiResponse({ status: 200, description: 'OK', type: CursorResult<PinterestContentModel> })
  ...
  public async Contents(
    ...
    const result = await this.commandBus.execute(new PinterestContentsQuery({
      model: { userId, cursor },
    }));
