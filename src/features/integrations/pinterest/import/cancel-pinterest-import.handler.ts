...
export class CancelPinterestImportCommandHandler implements ICommandHandler<CancelPinterestImportCommand> {
  ...
  public async execute(command: CancelPinterestImportCommand): Promise<void> {
    ...
    const importNotification = notifications.find(
      n => n.type === NotificationType.Import && n.isLive && n.metaData?.platform === _const.PLATFORMS.PINTEREST,
    );
    ...
    logger.info(`[PinterestImport] Cancellation flag set for user ${userId}. Triggering immediate rollback.`);

    try {
      this.eventEmitter.emit('platform.rollback', new PlatformRollbackEvent({ account }));
      logger.info(`[PinterestImport] Rollback event emitted for user ${userId}`);
    } catch (error) {
      logger.error(`[PinterestImport] Error emitting rollback event: 
