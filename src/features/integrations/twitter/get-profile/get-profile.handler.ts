...
export class TwitterProfileQueryHandler implements ICommandHandler<TwitterProfileQuery> {
  ...
  public async execute(query: TwitterProfileQuery): Promise<TwitterProfileModel> {
    ...
    const account = model.userId ? await this.linkedAccountRepository.getByPlatformAndUserIdAsync(PLATFORM, model.userId)
      : model.userName ? await this.linkedAccountRepository.getByPlatformAndUserNameAsync(PLATFORM, model.userName)
        : await this.linkedAccountRepository.getByPlatformAndExternalIdAsync(PLATFORM, model.twitterId);

    if (!account) {
      throw new NotFoundException("No matching Twitter profile was found based on the provided information.");
    }
