...
export class LinkedInProfileQueryHandler implements ICommandHandler<LinkedInProfileQuery> {
  ...
  public async execute(command: LinkedInProfileQuery): Promise<LinkedInProfileModel> {
    ...
    const account = model.userId ? await this.linkedAccountRepository.getByPlatformAndUserIdAsync(PLATFORM, model.userId)
      : model.userName ? await this.linkedAccountRepository.getByPlatformAndUserNameAsync(PLATFORM, model.userName)
        : await this.linkedAccountRepository.getByPlatformAndExternalIdAsync(PLATFORM, model.linkedInId);

    if (!account) {
      throw new NotFoundException("No matching LinkedIn profile was found based on the provided information.");
    }
