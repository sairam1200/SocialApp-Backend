...
export class FacebookAnalyticsController {
  ...
  public async sync(@Res() res: Response): Promise<Response> {
    const result = await this.commandBus.execute(
      new SyncFacebookAnalyticsCommand(),
    );
  ...
  public async getPageAnalytics(@Res() res: Response): Promise<Response> {
    const result = await this.queryBus.execute(
      new GetFacebookPageAnalyticsQuery(),
    );
    return res.status(HttpStatus.OK).json(result);
  }
  ...
  public async getGrowth(
    ...
    const result = await this.queryBus.execute(
      new GetFacebookGrowthQuery({ startDate, endDate }),
    );
    return res.status(HttpStatus.OK).json(result);
  }
  ...
  public async getPostAnalytics(
    ...
    const result = await this.queryBus.execute(
      new GetFacebookPostAnalyticsQuery(id),
    );
    return res.status(HttpStatus.OK).json(result);
  }
  ...
  public async getVideoAnalytics(
    ...
    const result = await this.queryBus.execute(
      new GetFacebookVideoAnalyticsQuery(id),
    );
    return res.status(HttpStatus.OK).json(result);
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Top performing posts ordered by reach',
  })
  @ApiResponse({ status: 401, description: 'UNAUTHORIZED' })
  public async getTopPosts(
    ...
    const result = await this.queryBus.execute(
      new GetFacebookTopPostsQuery({ limit }),
    );
    return res.status(HttpStatus.OK).json(result);
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Top performing videos ordered by views',
  })
  @ApiResponse({ status: 401, description: 'UNAUTHORIZED' })
  public async getTopVideos(
    ...
    const result = await this.queryBus.execute(
      new GetFacebookTopVideosQuery({ limit }),
    );
    return res.status(HttpStatus.OK).json(result);
  }
  ...
  @ApiResponse({
    status: 200,
    description: 'Compare analytics metrics between two custom date ranges',
  })
  @ApiResponse({ status: 401, description: 'UNAUTHORIZED' })
  @ApiResponse({ status: 404, description: 'NOT_FOUND' })
