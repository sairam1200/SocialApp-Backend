...
import { DiscordProfileModel, DiscordUserDataType } from "../../../../domain/contracts/discord.model";
...
export class DiscordConnectCallbackQueryHandler implements ICommandHandler<DiscordConnectCallbackQuery> {
  ...
  public async execute(query: DiscordConnectCallbackQuery): Promise<{
    ...
    let linkedAccount = await this.linkedAccountRepository.getByPlatformAndUserIdAsync(_const.PLATFORMS.DISCORD, user.id);
    const newExternalId = String(userData.id);
    if (linkedAccount) {
      const oldExternalId = linkedAccount.externalId;

      if (oldExternalId !== newExternalId) {
        logger.info(`[DiscordConnect] User ${user.id} changed Discord account from ${oldExternalId} to ${newExternalId}`);
    ...
    const existingAccountLogin = await this.userLoginRepository.getByUserIdAndProviderAsync(user.id, _const.PLATFORMS.DISCORD);
  ...
  private async fetchToken(code: string)
    ...
    try {
      ...
      if (response.data.error) {
        ...
        throw new ApplicationException(`Discord OAuth error: ${response.data.error_description || response.data.error}`);
      }
      return response.data;
    } catch (error) {
      if (error instanceof ApplicationException) throw error;
      logger.error('Error fetching token from Discord', error);
      throw new ApplicationException('Unexpected error during authentication with Discord');
    }
  }

  private async fetchUserData(accessToken: string): Promise<DiscordUserDataType> {
    try {
      const response = await axios.get<DiscordUserDataType>(`${DISCORD_API_URL}/users/@me`, {
        headers: {
  ...
  private getAvatarUrl(userData: DiscordUserDataType): string {
    ...
    const index = userData.discriminator === '0'
      ? (BigInt(userData.id) >> BigInt(22)) % BigInt(6)
      : Number(userData.discriminator) % 5;
    return `https://cdn.discordapp.com/embed/avatars/${index}.png`;
  }

  private async updateLinkedAccount(linkedAccount: LinkedAccount, userData: DiscordUserDataType): Promise<LinkedAccount> {
  ...
  private async createLinkedAccount(userId: string, userData: DiscordUserDataType): Promise<LinkedAccount> {
    await this.contentStreamRepository.deleteByPlatformAndExternalIdAsync(
      _const.PLATFORMS.DISCORD,
      String(userData.id),
    );
