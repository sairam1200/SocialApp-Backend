...
export class EnableSnapchatSyncCommandHandler implements ICommandHandler<EnableSnapchatSyncCommand> {
  ...
  public async execute(command: EnableSnapchatSyncCommand): Promise<{ syncEnabled: boolean }> {
    ...
    logger.info(`[SnapchatSync] Sync enabled for user ${userId}. Triggering initial import.`);

    const userLogin = await this.userLoginRepository.getByUserIdAndProviderAsync(userId, _const.PLATFORMS.SNAPCHAT);
    if (userLogin) {
      const tokenValue = deserializeObject<{ access_token: string }>(userLogin.tokenValue);
      await this.queueService.enqueueSnapchatImport(account, tokenValue.access_token || userLogin.tokenValue);
      logger.info(`[SnapchatSync] Import job enqueued for user ${userId}`);
    } else {
      logger.warn(`[SnapchatSync] UserLogin not found for user ${userId}. Cannot trigger import after sync enable.`);
    }
