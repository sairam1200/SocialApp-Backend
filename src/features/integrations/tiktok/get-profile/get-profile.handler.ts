...
export class TiktokProfileQueryHandler implements ICommandHandler<TiktokProfileQuery> {
  ...
  public async execute(query: TiktokProfileQuery): Promise<TiktokProfileModel> {
    ...
    const account = model.userId ? await this.linkedAccountRepository.getByPlatformAndUserIdAsync(_const.PLATFORMS.TIKTOK, model.userId)
      : model.userName ? await this.linkedAccountRepository.getByPlatformAndUserNameAsync(_const.PLATFORMS.TIKTOK, model.userName)
        : await this.linkedAccountRepository.getByPlatformAndExternalIdAsync(_const.PLATFORMS.TIKTOK, model.tiktokId);

    if (!account) {
      throw new NotFoundException("No matching tiktok profile was found based on the provided information.");
    }
    ...
    const includeSensitiveFields = HttpContext.user
      ? (account.userId === HttpContext.user[Globals.ClaimTypes.UserId]
        || HttpContext.user.permission.some((a: string) => a === "viewuser")) : false;
