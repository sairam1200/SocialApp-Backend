...
import { TiktokContentsQuery } from "./get-contents.handler";
...
export class TiktokContentsController {
  ...
  @ApiResponse({ status: 200, description: 'OK', type: CursorResult<TikTokContentModel> })
  ...
  public async Contents(
    ...
    const result = await this.commandBus.execute(new TiktokContentsQuery({
      model: { userId, cursor },
    }));
