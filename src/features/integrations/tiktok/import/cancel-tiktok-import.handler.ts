...
export class CancelTiktokImportCommandHandler implements ICommandHandler<CancelTiktokImportCommand> {
  ...
  public async execute(command: CancelTiktokImportCommand): Promise<void> {
    ...
    const importNotification = notifications.find(
      n => n.type === NotificationType.Import && n.isLive && n.metaData?.platform === _const.PLATFORMS.TIKTOK,
    );
    ...
    logger.info(`[TiktokImport] Cancellation flag set for user ${userId}. Triggering immediate rollback.`);

    try {
      this.eventEmitter.emit('platform.rollback', new PlatformRollbackEvent({ account }));
      logger.info(`[TiktokImport] Rollback event emitted for user ${userId}`);
    } catch (error) {
      logger.error(`[TiktokImport] Error emitting rollback event: 
