...
import { ThreadsProfileModel, ThreadsUserDataType } from "../../../../domain/contracts/threads.model";
...
export class ThreadsConnectCallbackQueryHandler implements ICommandHandler<ThreadsConnectCallbackQuery> {
  ...
  public async execute(query: ThreadsConnectCallbackQuery):
    Promise<{ accessToken: string; expiresIn: number; profile: ThreadsProfileModel; }> {
    const { model } = query;
    ...
    try {
      ...
      throw new ApplicationException('Threads API is not yet available. Please check back later.');
    }
    ...
    let linkedAccount = await this.linkedAccountRepository.getByPlatformAndUserIdAsync(_const.PLATFORMS.THREADS, user.id);
    const newExternalId = userData.id;
    if (linkedAccount) {
      const oldExternalId = linkedAccount.externalId;

      if (oldExternalId !== newExternalId) {
        logger.info(`[ThreadsConnect] User ${user.id} changed Threads account from ${oldExternalId} to ${newExternalId}`);
    ...
    let existingAccountLogin = await this.userLoginRepository.getByUserIdAndProviderAsync(user.id, _const.PLATFORMS.THREADS);
  ...
  private async fetchToken(code: string)
    ...
    const basicAuth = Buffer.from(`${configs.threads?.clientId || ''}:${configs.threads?.clientSecret || ''}`).toString('base64');
    try {
  ...
  private async fetchUserData(accessToken: string): Promise<ThreadsUserDataType> {
    try {
