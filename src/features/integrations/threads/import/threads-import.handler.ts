  ...
  implements ICommandHandler<ThreadsImportCommand>
{
  ...
  public async execute(
    ...
    if (threadsAccessToken) {
      const isTokenValid =
        await this.verifyAccessTokenAsync(threadsAccessToken);
      if (!isTokenValid) {
    ...
    try {
      ...
      logger.error(
        `An error occurred while enqueueing the Threads import job: 
        ${error instanceof Error ? error.message : JSON.stringify(error)}`,
        { error },
      );
      throw new ApplicationException(
        'Failed to initiate Threads import. Please try again later.',
      );
    }
    return {
      accessToken,
  ...
  private async verifyAccessTokenAsync(accessToken: string): Promise<boolean> {
    try {
      ...
      logger.error(
        'Threads access token verification failed:',
