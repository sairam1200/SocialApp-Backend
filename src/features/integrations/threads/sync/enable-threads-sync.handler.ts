...
export class EnableThreadsSyncCommandHandler implements ICommandHandler<EnableThreadsSyncCommand> {
  ...
  public async execute(command: EnableThreadsSyncCommand): Promise<{ syncEnabled: boolean }> {
    ...
    logger.info(`[ThreadsSync] Sync enabled for user ${userId}. Triggering initial import.`);
