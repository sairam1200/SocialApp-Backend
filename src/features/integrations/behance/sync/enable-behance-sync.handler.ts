...
export class EnableBehanceSyncCommandHandler implements ICommandHandler<EnableBehanceSyncCommand> {
  ...
  public async execute(
    command: EnableBehanceSyncCommand,
    ...
    logger.info(
      `[BehanceSync] Sync enabled for user ${userId}. Triggering initial import.`,
    );
    ...
    if (userLogin) {
      ...
      logger.warn(
        `[BehanceSync] UserLogin not found for user ${userId}. Cannot trigger import after sync enable.`,
      );
    }
