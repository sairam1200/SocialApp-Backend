...
export class InstagramProfileQueryHandler implements ICommandHandler<InstagramProfileQuery> {
  ...
  public async execute(command: InstagramProfileQuery): Promise<InstagramProfileModel> {
    ...
    const account = model.userId ? await this.linkedAccountRepository.getByPlatformAndUserIdAsync(PLATFORM, model.userId)
      : model.userName ? await this.linkedAccountRepository.getByPlatformAndUserNameAsync(PLATFORM, model.userName)
        : await this.linkedAccountRepository.getByPlatformAndExternalIdAsync(PLATFORM, model.instagramId);

    if (!account) {
      throw new NotFoundException("No matching Instagram profile was found based on the provided information.");
    }
