...
import {
  ...
  YoutubeImportRequestModel,
} from './youtube-import.handler';
import {
  CancelYoutubeImportCommand,
  CancelYoutubeImportRequestModel,
} from './cancel-youtube-import.handler';
...
export class YoutubeImportController {
  ...
  public async Import(
    ...
    if (model.youtubeAccessToken) {
      return res
        ...
        .json({ message: 'Youtube import has begun.' });
    }

    return res
      .status(HttpStatus.OK)
      .json({ message: 'Youtube import has begun.', ...result });
  }
  ...
  public async Cancel(
    ...
    return res
      .status(HttpStatus.OK)
      .json({ message: 'Youtube import cancellation and rollback requested.' });
  }
}
