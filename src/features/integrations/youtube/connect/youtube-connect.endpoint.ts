...
import {
  YoutubeConnectCallbackQuery,
  YoutubeConnectQuery,
} from './youtube-connect.handler';
...
export class YoutubeConnectController {
  ...
  public async Connect(@Res() res: Response): Promise<Response | void> {
    ...
    await this.commandBus.execute(
      new YoutubeConnectQuery({ model: { state } }),
    );
  ...
  @ApiResponse({
    status: 200,
    description: 'OK',
    type: YoutubeConnectCallbackResponseModel,
  })
  @ApiResponse({ status: 401, description: 'UNAUTHORIZED' })
  @ApiResponse({ status: 400, description: 'BAD_REQUEST' })
  @ApiResponse({ status: 403, description: 'FORBIDDEN' })
  public async Callback(
    ...
    const result = await this.commandBus.execute(
      new YoutubeConnectCallbackQuery({
        model: { code, state },
      }),
    );
