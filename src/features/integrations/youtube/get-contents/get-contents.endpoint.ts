...
import { YoutubeContentsQuery } from "./get-contents.handler";
...
export class YoutubeContentsController {
  ...
  @ApiResponse({ status: 200, description: 'OK', type: CursorResult<YouTubeContentModel> })
  ...
  public async Contents(
    ...
    const result = await this.commandBus.execute(new YoutubeContentsQuery({
      model: { userId, cursor },
    }));
