  ...
  implements ICommandHandler<YoutubeDisconnectCommand>
{
  ...
  public async execute(command: YoutubeDisconnectCommand): Promise<void> {
    ...
    if (!linkedAccount) {
      logger.warn(
        `[YoutubeDisconnect] No LinkedAccount found for user ${userId}`,
      );
      return;
    }
    ...
    logger.info(
      `[YoutubeDisconnect] Starting disconnect for user ${userId}, channel ${channelId || 'unknown'}`,
    );
      ...
      try {
        ...
        if (access_token) {
          this.revokeGoogleToken(access_token).catch((err) =>
            logger.error(
              `[YoutubeDisconnect] Token revocation failed: ${err.message}`,
            ),
          );
        }
      } catch {
        logger.warn(
          `[YoutubeDisconnect] Could not parse tokenValue for revocation`,
        );
      }
    }
    ...
    if (channelId && configs.youtube.webhookUrl) {
      this.youtubeWebhookService
        .unsubscribeAsync(channelId, configs.youtube.webhookUrl)
        .catch((err) =>
          logger.error(
            `[YoutubeDisconnect] Webhook unsubscribe failed: ${err.message}`,
          ),
        );
    }
    this.queueService
      .cancelYoutubeImport(userId)
      .catch((err) =>
        logger.error(
          `[YoutubeDisconnect] Cancel import job failed: ${err.message}`,
        ),
      );

    // Step 4: Collect video IDs for queue cancellation (before transaction)
    let videoIds: string[] = [];
    const youtubeAccount = await this.dataSource
      .getRepository(YoutubeAccount)
      .findOne({ where: { userId } });
    if (youtubeAccount) {
      const videos = await this.dataSource
        .getRepository(YoutubeVideo)
        .find({ where: { accountId: youtubeAccount.id } });
    ...
    if (videoIds.length > 0) {
      this.queueService
        .cancelYoutubeUpload(videoIds)
        .catch((err) =>
          logger.error(
            `[YoutubeDisconnect] Cancel upload jobs failed: ${err.message}`,
          ),
        );
    }
    ...
    await this.dataSource.transaction(async (entityManager) => {
      ...
      const acct = youtubeAccount
        ? await entityManager
            .getRepository(YoutubeAccount)
            .findOne({ where: { id: youtubeAccount.id } })
        : null;
        ...
        if (videoIds.length > 0) {
          await Promise.all([
            entityManager
              ...
              .delete({ videoId: In(videoIds) }),
            entityManager
              .getRepository(YoutubeVideo)
              .delete({ accountId: acct.id }),
          ]);
        }
    ...
    logger.info(
      `[YoutubeDisconnect] Full disconnect complete for user ${userId}`,
    );
  }
