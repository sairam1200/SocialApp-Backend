...
import {
  PermissionModel,
  RoleClaimModel,
} from '../../../domain/contracts/role.model';
  ...
  implements ICommandHandler<GetPermissionsQuery>
{
  ...
  public async execute(query: GetPermissionsQuery): Promise<PermissionModel> {
    ...
    const result = {} as PermissionModel;

    const role =
      (await this.roleRepository.getByIdAsync(query.roleId)) ??
      (() => {
    ...
    const allPermissions: string[] =
      this.permissions.discoverControllerPermissions();
    const roleClaims = await this.roleClaimRepository.getByRoleIdAsync(role.id);
    const roleClaimsValue: string[] = roleClaims.map(
