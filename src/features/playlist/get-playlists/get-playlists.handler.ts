...
export class GetPlaylistsQueryHandler implements ICommandHandler<GetPlaylistsQuery, PlaylistModel[]> {
  constructor(
  ...
  public async execute(query: GetPlaylistsQuery): Promise<PlaylistModel[]> {
    ...
    const playlists = await this.playlistRepository.getAsync(model.userNameOrId);
