...
@Processor(
  _const.BULL_QUEUES.THREADS_IMPORT,
  BullMQConfig.getWorkerOptions(_const.BULL_QUEUES.THREADS_IMPORT, 2),
)
export class ThreadsImportProcessor extends WorkerHost {
  ...
  public async process(job: Job<ThreadsImportJobData>): Promise<void> {
    ...
    if (!currentAccount) {
      logger.error(
        `[ThreadsImport] Account not found for user ${account.userId}`,
      );
      return;
    }
    ...
    if (!(await job.isActive())) {
      logger.info(
        `[ThreadsImport] Job ${job.id} is no longer active, stopping import for user ${account.userId}`,
      );
      return;
    }
    ...
    logger.warn(
      `[ThreadsImport] Threads API is not yet available. Structure ready for future implementation.`,
    );
