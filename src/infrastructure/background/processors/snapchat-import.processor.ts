...
@Processor(
  _const.BULL_QUEUES.SNAPCHAT_IMPORT,
  BullMQConfig.getWorkerOptions(_const.BULL_QUEUES.SNAPCHAT_IMPORT, 2),
)
export class SnapchatImportProcessor extends WorkerHost {
  ...
  public async process(job: Job<SnapchatImportJobData>): Promise<void> {
    ...
    if (!currentAccount) {
      logger.error(
        `[SnapchatImport] Account not found for user ${account.userId}`,
      );
      return;
    }
    ...
    if (!(await job.isActive())) {
      logger.info(
        `[SnapchatImport] Job ${job.id} is no longer active, stopping import for user ${account.userId}`,
      );
      return;
    }
    ...
    logger.warn(
      `[SnapchatImport] Snapchat API integration not yet fully implemented. Using fallback logic.`,
    );
