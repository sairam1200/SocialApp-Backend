...
export class TiktokImportProcessor extends WorkerHost {
  ...
  public async process(job: Job<TiktokImportJobData>): Promise<void> {
    ...
    if (!(await job.isActive())) {
      logger.info(`[TiktokImport] Job ${job.id} is no longer active, stopping import for user ${account.userId}`);
      return;
    }
    ...
    for (const [key, { endpoint, type }] of Object.entries(fields)) {
      ...
      try {
        while (true) {
          if (!(await job.isActive())) {
            logger.info(`[TiktokImport] Job ${job.id} cancelled during processing ${type}`);
          ...
          for (const item of items) {
            ...
            try {
              ...
              this.gateway.emitNewImportContent(account.userId, _const.PLATFORMS.TIKTOK, mappedContent);
    ...
    if (!(await job.isActive())) {
      logger.info(`[TiktokImport] Job ${job.id} was cancelled, rolling back imported content`);
      ...
      if (importedExternalIds.length > 0) {
        try {
          ...
          logger.info(`[TiktokImport] Rolled back ${importedExternalIds.length} imported items for user ${account.userId}`);
        } catch (rollbackError) {
