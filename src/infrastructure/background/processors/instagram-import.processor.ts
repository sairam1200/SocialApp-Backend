...
@Processor(
  _const.BULL_QUEUES.INSTAGRAM_IMPORT,
  BullMQConfig.getWorkerOptions(_const.BULL_QUEUES.INSTAGRAM_IMPORT, 2),
)
export class InstagramImportProcessor extends WorkerHost {
  ...
  public async process(job: Job<InstagramImportJobData>): Promise<void> {
    ...
    if (!currentAccount) {
      logger.error(
        `[InstagramImport] Account not found for user ${account.userId}`,
      );
      return;
    }
    ...
    if (!(await job.isActive())) {
      logger.info(
        `[InstagramImport] Job ${job.id} is no longer active, stopping import for user ${account.userId}`,
      );
      return;
    }
    ...
    const instagramBusinessAccountId =
      account.metaData?.instagram_business_account_id || account.externalId;
    const fields: Record<string, { endpoint: string; type: string }> = {
      media: {
        endpoint: `/${instagramBusinessAccountId}/media`,
        type: 'Media',
      },
    };
    ...
    for (const [key, { endpoint, type }] of Object.entries(fields)) {
      ...
      try {
        while (true) {
          if (!(await job.isActive())) {
            logger.info(
              `[InstagramImport] Job ${job.id} cancelled during processing ${type}`,
            );
          ...
          logger.debug(
            `[InstagramImport] Retrieved ${items.length} ${type} items`,
          );
          ...
          for (const item of items) {
            ...
            content.publishedAt = item.timestamp
            ...
            try {
              ...
              logger.error(
                `[InstagramImport] Error saving ${type} content:`,
                err.message,
              );
            }
          }
          ...
          if (data.paging?.cursors?.after) {
            ...
            logger.info(
              `[InstagramImport] Completed import of ${type} for user ${account.userId}`,
            );
            progressReports[type].status = NotificationStatus.Completed;
            delete lastCursors[key];
            await this.saveCursors(currentAccount, lastCursors);
        ...
        progressReports[type].status = NotificationStatus.Cancelled;
        logger.error(
          `[InstagramImport] Error occurred while importing ${type}:`,
          err.message,
        );
        if (cursor) {
          lastCursors[key] = cursor;
    ...
    if (!(await job.isActive())) {
      logger.info(
        `[InstagramImport] Job ${job.id} was cancelled, rolling back imported content`,
      );
      ...
      if (importedExternalIds.length > 0) {
        try {
          ...
          logger.info(
            `[InstagramImport] Rolled back ${importedExternalIds.length} imported items for user ${account.userId}`,
          );
        } catch (rollbackError) {
          logger.error(
            `[InstagramImport] Error during rollback:`,
            rollbackError,
          );
        }
      }
    ...
    if (notification) {
      if (encounteredError) {
        logger.warn(
          `[InstagramImport] Completed with issues for user ${account.userId}`,
        );
        await this.notificationService.updateAsync(
          notification.id,
          false,
        ...
        logger.info(
          `[InstagramImport] Successfully completed import for user ${account.userId}`,
        );
        await this.notificationService.updateAsync(
          notification.id,
          false,
      ...
      logger.warn(
        `[InstagramImport] No notification initialized for user ${account.userId}`,
      );
    }
  }
