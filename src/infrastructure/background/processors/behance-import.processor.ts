...
@Processor(
  _const.BULL_QUEUES.BEHANCE_IMPORT,
  BullMQConfig.getWorkerOptions(_const.BULL_QUEUES.BEHANCE_IMPORT, 2),
)
export class BehanceImportProcessor extends WorkerHost {
  ...
  public async process(job: Job<BehanceImportJobData>): Promise<void> {
    ...
    if (!currentAccount) {
      logger.error(
        `[BehanceImport] Account not found for user ${account.userId}`,
      );
      return;
    }
    ...
    if (!(await job.isActive())) {
      logger.info(
        `[BehanceImport] Job ${job.id} is no longer active, stopping import for user ${account.userId}`,
      );
      return;
    }
    ...
    logger.warn(
      `[BehanceImport] Behance API not available. Using fallback logic with limited functionality.`,
    );
