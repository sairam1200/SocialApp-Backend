...
export class InstagramImportListener {
  ...
  constructor(
    ...
    logger.info(`[InstagramImport] Listener initialized`);
  }

  @OnEvent('instagram.import', { async: true })
  async handleInstagramImport(event: InstagramImportEvent): Promise<void> {
    ...
    if (currentAccount.metaData?.importCancelled) {
      logger.info(`[InstagramImport] Import cancelled for user ${account.userId}`);
      ...
      try {
        while (true) {
          ...
          if (checkAccount?.metaData?.importCancelled) {
            logger.info(`[InstagramImport] Import cancelled during processing ${type}`);
          ...
          for (const item of items) {
            ...
            try {
              ...
              this.gateway.emitNewImportContent(account.userId, _const.PLATFORMS.INSTAGRAM, savedContent);
    ...
    if (finalAccount?.metaData?.importCancelled) {
      logger.info(`[InstagramImport] Import was cancelled, cleaning up`);
      ...
      if (notification) {
        ...
        await this.notificationService.updateAsync(
          ...
          "❌ Instagram import was cancelled",
        );
      }
      return;
    }
    ...
    if (notification) {
      if (encounteredError) {
        ...
        await this.notificationService.updateAsync(
          ...
          "⚠️ Instagram import completed with issues",
        );
      } else {
      ...
      await this.notificationService.notifyAsync(
        ...
        "⚠️ Instagram import could not start",
