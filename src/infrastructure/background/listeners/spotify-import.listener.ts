...
export class SpotifyImportListener {
  ...
  constructor(
    ...
    logger.info(`[SpotifyImport] Listener initialized`);
  }

  @OnEvent('spotify.import', { async: true })
  async handleSpotifyImport(event: SpotifyImportEvent): Promise<void> {
    ...
    if (currentAccount.metaData?.importCancelled) {
      logger.info(`[SpotifyImport] Import cancelled for user ${account.userId}`);
      ...
      try {
        while (true) {
          ...
          if (checkAccount?.metaData?.importCancelled) {
            logger.info(`[SpotifyImport] Import cancelled during processing ${type}`);
    ...
    if (finalAccount?.metaData?.importCancelled) {
      logger.info(`[SpotifyImport] Import was cancelled, cleaning up`);
