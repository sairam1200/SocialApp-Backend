...
export class TwitterImportListener {
  ...
  constructor(
    ...
    logger.info(`[TwitterImport] Listener initialized`);
  }

  @OnEvent('twitter.import', { async: true })
  async handleTwitterImport(event: TwitterImportEvent): Promise<void> {
    ...
    if (currentAccount.metaData?.importCancelled) {
      logger.info(`[TwitterImport] Import cancelled for user ${account.userId}`);
      ...
      try {
        while (true) {
          ...
          if (checkAccount?.metaData?.importCancelled) {
            logger.info(`[TwitterImport] Import cancelled during processing ${type}`);
    ...
    if (finalAccount?.metaData?.importCancelled) {
      logger.info(`[TwitterImport] Import was cancelled, cleaning up`);
