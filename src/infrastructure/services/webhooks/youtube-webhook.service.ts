...
export class YoutubeWebhookService {
  ...
  public async subscribeAsync(
    ...
    try {
      ...
      logger.info(
        `[YoutubeWebhook] Subscription request sent for channel ${channelId}`,
      );
    } catch (error: any) {
      logger.error(
  ...
  public async unsubscribeAsync(
    ...
    try {
      ...
      logger.info(
        `[YoutubeWebhook] Unsubscription request sent for channel ${channelId}`,
      );
    } catch (error) {
      logger.error(
        `[YoutubeWebhook] Error unsubscribing from channel ${channelId}:`,
        error,
      );
      throw error;
    }
