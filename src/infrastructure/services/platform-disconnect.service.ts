...
export class PlatformDisconnectService implements IPlatformDisconnectService {
  ...
  public async disconnectPlatformAsync(
    ...
    await Promise.all([
      this.backupUserContentToContentStreamAsync(
        userId,
        platform,
        userContents,
        entityManager,
    ...
    if (entityManager) {
      if (userContents.length > 0) {
        ...
        await entityManager
          ...
          .delete({ userId, platform, externalId: In(externalIds) });
      }
      if (userLogin) {
      ...
      const deletePromises: Promise<void | LinkedAccount>[] = [
        this.userContentRepository.deleteByUserIdAndPlatformAsync(
          userId,
          platform,
        ),
      ];
      ...
      deletePromises.push(
        this.linkedAccountRepository.deleteAsync(linkedAccount),
      );
      await Promise.all(deletePromises);
    }

    logger.info(
      `[PlatformDisconnect] Successfully disconnected platform ${platform} for user ${userId} - backed up ${userContents.length} content items and profile`,
    );
  }

  private async fetchAllUserContentAsync(
    userId: string,
    platform: string,
  ): Promise<UserContent[]> {
  ...
  private async backupUserContentToContentStreamAsync(
    ...
    if (entityManager) {
      await entityManager
        ...
        .delete({ platform, externalId: In(externalIds) });
    } else {
      await this.contentStreamRepository.deleteByPlatformAndExternalIdsAsync(
        platform,
        externalIds,
      );
    }

    const contentStreams = userContents.map(
      (userContent) =>
        new ContentStream({
    ...
    logger.info(
      `[PlatformDisconnect] Backed up ${contentStreams.length} UserContent records to ContentStream for platform ${platform}`,
    );
  }

  private async backupProfileToContentStreamAsync(
    ...
    if (entityManager) {
      await entityManager.getRepository(ContentStream).delete({
        platform: linkedAccount.platform,
        externalId: linkedAccount.externalId,
      });
