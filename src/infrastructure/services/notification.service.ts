...
export class NotificationService implements INotificationService {
  ...
  public async notifyAsync(
    ...
    const notification = await this.notificationRepository.createAsync(
      new Notification({
        body: body,
        type: type,
        title: title,
    ...
    const shouldNotifyInApp =
      !channels ||
      channels.length === 0 ||
      channels.includes(NotificationChannel.InApp);

    if (shouldNotifyInApp) {
      this.gateway.emitNewNotification(
        userId,
        mapToNotificationModel(notification),
      );
    }
    return notification;
  }
  ...
  public async updateAsync(
    ...
    await this.notificationRepository.updateAsync(notification);
    this.gateway.emitNotificationUpdated(
      notification.notifyId,
      mapToNotificationModel(notification),
    );
  }

  public async markSoundAsPlayedAsync(
    id: string,
    userId?: string,
  ): Promise<void> {
    if (!userId) {
