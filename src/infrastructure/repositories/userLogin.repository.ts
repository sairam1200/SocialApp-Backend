...
export class UserLoginRepository implements IUserLoginRepository {
  ...
  public async getByUserIdAndProviderAsync(
    userId: string,
    provider: string,
  ): Promise<UserLogin> {
  ...
  public async getByUserIdAndDeviceIdAsync(
    userId: string,
    deviceId: string,
  ): Promise<UserLogin> {
  ...
  public async getByTokenValueAndDeviceIdAsync(
    tokenValue: string,
    deviceId: string,
  ): Promise<UserLogin> {
    return await this.userLoginContext.findOne({
      where: { tokenValue, deviceId },
    });
  }
