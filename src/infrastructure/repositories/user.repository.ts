...
export class UserRepository implements IUserRepository {
  ...
  public async searchGlobalAsync(keyword: string, viewerUserId: string, page: number, limit: number): Promise<[SearchUserProjection[], number]> {
    ...
    const qb = this.userContext.createQueryBuilder("user")
      ...
      .addSelect(`(SELECT COALESCE(json_agg(json_build_object('id', la.id, 'platform', la.platform, 'verified', la.verified) ORDER BY la.platform) FILTER (WHERE la.id IS NOT NULL), '[]'::json) FROM "linkedAccounts" la WHERE la."userId" = user.id)`, "linkedAccounts")
      .addSelect(`(SELECT EXISTS(SELECT 1 FROM "linkedAccounts" la WHERE la."userId" = user.id AND la.verified = true))`, "verified")
