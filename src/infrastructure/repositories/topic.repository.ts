...
export class TopicRepository implements ITopicRepository {
  ...
  public async createUserTopicsAsync(userId: string, topicIds: string[]): Promise<void> {
  ...
  public async deleteUserTopicsAsync(userId: string, topicIds: string[]): Promise<void> {
    if (!topicIds || topicIds.length === 0) {
      return;
    }
