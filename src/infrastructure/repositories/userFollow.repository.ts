...
export class UserFollowRepository implements IUserFollowRepository {
  constructor(
    @InjectRepository(UserFollow) private readonly followContext: Repository<UserFollow>,
    @InjectRepository(User) private readonly userContext: Repository<User>,
  ) { }

  public async getAsync(followerId: string, followedId: string): Promise<UserFollow | null> {
  ...
  public async updateStatusAsync(followId: string, status: FollowStatus): Promise<boolean> {
    const result = await this.followContext.update(followId, { status });
    return result.affected > 0;
  }

  public async getWithUsersAsync(followerId: string, followedId: string): Promise<UserFollow | null> {
    const qb = this.buildFollowQuery('follow');
    qb.where('follow.followerId = :followerId', { followerId })
      .andWhere('follow.followedId = :followedId', { followedId });
    return qb.getOne();
  }

  public async getFollowersAsync(userId: string, status: FollowStatus | null): Promise<UserFollow[]> {
    const qb = this.buildFollowQuery('follow');
  ...
  public async getFollowingAsync(userId: string, status: FollowStatus | null): Promise<UserFollow[]> {
    const qb = this.buildFollowQuery('follow');
  ...
  public async countFollowingAsync(userId: string, status?: FollowStatus): Promise<number> {
    const qb = this.followContext.createQueryBuilder('follow')
  ...
  public async getCommonFollowersAsync(userAId: string, userBId: string, status?: FollowStatus): Promise<UserFollow[]> {
    const qb = this.followContext.createQueryBuilder('follow')
      .leftJoinAndSelect('follow.follower', 'follower')
      .where('follow.followedId IN (:...userIds)', { userIds: [userAId, userBId] });
    ...
    qb.select([
      'follow.followerId as followerId',
