...
export class OnboardingFeature1769000000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "identity"."users_onboardingstep_enum" AS ENUM('NotStarted', 'ProfileData', 'Topics', 'Platforms', 'Confirmation', 'Completed')`,
    );

    // Add onboardingStep column to users table (profileImage already exists from previous migration)
    await queryRunner.query(
      `ALTER TABLE "identity"."users" ADD "onboardingStep" "identity"."users_onboardingstep_enum" DEFAULT 'NotStarted'`,
    );
    ...
    // Create indexes
    await queryRunner.query(
      `CREATE INDEX "idx_userTopics_user" ON "userTopics" ("userId")`,
    );
    await queryRunner.query(
      `CREATE INDEX "idx_userTopics_topic" ON "userTopics" ("topicId")`,
    );

    // Add foreign key constraints
    await queryRunner.query(
      `ALTER TABLE "userTopics" ADD CONSTRAINT "FK_userTopics_user" FOREIGN KEY ("userId") REFERENCES "identity"."users"("id") ON DELETE CASCADE`,
    );
    await queryRunner.query(
      `ALTER TABLE "userTopics" ADD CONSTRAINT "FK_userTopics_topic" FOREIGN KEY ("topicId") REFERENCES "topics"("id") ON DELETE CASCADE`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    // Drop foreign keys
    await queryRunner.query(
      `ALTER TABLE "userTopics" DROP CONSTRAINT "FK_userTopics_topic"`,
    );
    await queryRunner.query(
      `ALTER TABLE "userTopics" DROP CONSTRAINT "FK_userTopics_user"`,
    );
    ...
    await queryRunner.query(
      `ALTER TABLE "identity"."users" DROP COLUMN "onboardingStep"`,
    );
