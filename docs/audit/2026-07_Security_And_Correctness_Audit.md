...
| [C4](#c4-oauth-tokens-encrypted-with-a-fixed-iv-and-no-authentication) | **High** | OAuth tokens encrypted with a fixed IV, unauthenticated CBC — ✅ **fixed** (AES-256-GCM, dual-read) |
| [C5](#c5-session-revocation-fails-open) | **High** | Session revocation fails open when Redis misses — ✅ **fixed** (DB fallback, fails closed) |
...
| [C4](#c4-oauth-tokens-encrypted-with-a-fixed-iv-and-no-authentication) Fixed IV / unauthenticated CBC | ✅ **Fixed** | `encrypt()` now emits **AES-256-GCM** as `v2:<iv>:<ciphertext>:<authTag>` with a fresh random 12-byte IV per message and an HKDF-derived key (domain-separated from the HMAC key). `decrypt()` routes by format so the hex CBC values already stored keep working — dual-read, no backfill, and legacy reads trend to zero as tokens are refreshed. Verified: non-deterministic output, tampering rejected on both ciphertext and auth tag, legacy values still readable. The old "documents the weakness" assertions are inverted, which was the planned signal. `timingSafeEqual` length check also retained. |
| [C5](#c5-session-revocation-fails-open) Revocation fails open | ✅ **Fixed** | `account.guard.ts` reads the authoritative stamps from the database on a cache miss, repopulates the cache, and compares — then **fails closed** if neither source can confirm the session. The database read had to come first: failing closed without it would have logged out every user with a cold cache. Guards gained an `IIdentityRepository` dependency; `authGuard.module` provides it. |
